#!/usr/bin/env node
/**
 * Generate a Word document (.docx) for human text editing from a PPTX extraction.
 *
  ...
  if (!hasAnyContent) {
    ...
    if (slide.body_shapes.length > 0) {
      ...
      if (slide.table) {
        ...
        // Measure the longest text in each column across all rows.
        ...
        // Ensure every column gets at least a minimum weight so short labels
        // aren't squished to zero.
        const MIN_WEIGHT = 5;
        ...
        // Fix rounding residual on the last column
                ...
                children: cellParas.length > 0
                  ? cellParas.map(p => new Paragraph({
                      ...
                      children: [new TextRun({
                        ...
                        font: "Calibri",
                        size: 20,
                        bold: ci === 0, // first column is typically a label
                      })]
                    }))
          ...
          const cardChildren = card.paragraphs.map((p, i) => {
            const opts = {
              ...
              children: [new TextRun({
                ...
                font: "Calibri",
                size: 20,
                bold: i === 0, // first text in card is the label/icon
              })]
            };
            if (p.bullet) {
              opts.numbering = { reference: "bullets", level: 0 };
