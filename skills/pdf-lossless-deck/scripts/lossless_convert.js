...
const { submitToNarakeet } = require('./narakeet_submit');

const SKILL_ROOT = path.join(__dirname, '..');

// Narration principles
const NARRATION_PRINCIPLES = fs.readFileSync(
  path.join(SKILL_ROOT, 'narration-principles.md'), 'utf8'
);

/**
 * Generate spoken narration for all pages in batched Claude API calls.
 */
async function generateNarration(pages, apiKey) {
  ...
  try {
    ...
    Anthropic = require(path.join(SKILL_ROOT, 'node_modules', '@anthropic-ai/sdk'));
  }
...
/**
 * Build narakeet.zip containing the script and all slide PNGs.
 */
async function buildZip(outputDir, pngPaths) {
  let archiver;
  try { archiver = require('archiver'); }
  catch { archiver = require(path.join(SKILL_ROOT, 'node_modules', 'archiver')); }
...
async function losslessConvert(pdfPath, outputDir, opts = {}) {
  ...
  if (!opts.noVideo) {
    ...
    t0 = t();
    videoPath = await submitToNarakeet(outputDir, { videoFilename });
