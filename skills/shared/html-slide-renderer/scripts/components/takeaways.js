...
module.exports = function makeTakeaways(ctx) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  function addKeyTakeaways(slide, pres, moduleNum, moduleTitle, pageNum, totalPages, takeaways) {
    addChrome(slide, pres, 'Key Takeaways', moduleNum, moduleTitle, pageNum, totalPages);

    takeaways.forEach((item, i) => {
      slide.add(`<div style="display:grid;grid-template-columns:${px(0.5)}px 1fr;gap:14px;align-items:start;">
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  function addRedFlagPairs(slide, flags) {
    const flagsHtml = flags.map(pair => `<div style="display:grid;grid-template-columns:1fr 1fr;gap:${px(0.3)}px;">
      ${[0, 1].map(side => {
